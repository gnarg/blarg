...
# [+exclude_extension+] An array of engine extensions not to run, <tt>[:migrations]</tt>
#
module Engines
  ...
  mattr_accessor :exclude_extensions
  ...
  class << self
    ...
    def load_extensions
      rails_extensions.each do|name|
        if !self.exclude_extensions.include? name.to_sym
          require "engines/rails_extensions/#{name}"
        end
      end
